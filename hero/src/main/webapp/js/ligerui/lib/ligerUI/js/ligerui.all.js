                    ...
                    if (g.trigger('beforeSetData', [data]) == false)
{
        ...
        setGrid: function (grid)
        {
            ...
            if(g.selectedValue&&g.selectedText){
            	valueArray=g.selectedValue.split(p.split);
            	textArray=g.selectedText.split(p.split);
            	if(valueArray.length==textArray.length){
            		for(var i=0;i<valueArray.length;i++){
            			var param={};
            			param[p.valueField]=valueArray[i];
            			param[p.textField]=textArray[i];
            			selecteds.push(param);
            		}
            	}
            ...
            if (p.condition)
            {
               ...
               var button= $("div", containerBtn1).ligerButton({
                ...
                var lastValue="";
                $("input",conditionPanel).keyup(function(e){
                	if($.trim(this.value)!=lastValue){
                		...
                		if (g.grid.get('url'))
                        {
                        	...
                        	for(var i=0;i<rules.length;i++){
                        	 ...
                        	 g.grid.set('newPage', 1);
                        	 g.grid.set('parms', param);
                        	 g.grid.reload();
                        } else
                        {
